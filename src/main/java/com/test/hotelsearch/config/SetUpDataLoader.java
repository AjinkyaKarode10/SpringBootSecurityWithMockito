...
ApplicationListener<ContextRefreshedEvent>  {
	...
	public void onApplicationEvent(ContextRefreshedEvent event) {
		...
		if(userRepository.findByUsername("Madhuri") != null)
		{
			return;
		}
...
//		alreadySetup = false;
		...
		Permission permission = createPermissionIfNotFound("UPDATE");
		Set<Permission> permissionSet = new HashSet<>(Arrays.asList(permission));
		...
		Role adminRole = roleRepo.findByName("ADMIN");
		User user1 = new User();
		user1.setEmail("madhu24@gmail.com");
		user1.setUsername("Madhuri");
		user1.setPassword(new BCryptPasswordEncoder().encode("madhuri"));
		user1.setRoles(new HashSet<Role>(Arrays.asList(adminRole)));
